...
void Signupwidget::on_addButton_clicked(){  //"添加"按钮点击
    ...
    QString control_str = QString("create table patients (id int primary key, name text,gender text, age int,times int)");
    sqlite.execute(control_str);
    sqlite.SigUserInsert(patientinfo);

    control_str = QString("create table p")+QString::number(id)+QString(" ( name text,position text, power int,\
