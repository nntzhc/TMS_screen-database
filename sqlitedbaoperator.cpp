...
bool SqliteDBAOperator::SigHisInsert(SchemeInfo singledb,int id,QString name,QString mode)
{
    QSqlQuery query;
    if (mode=="single")
    {  singledb.name="单脉冲";
    ...
    QString str=QString("INSERT INTO history VALUES ( :id,:name,:scheme, \
                        :position, :power,:threshold, \
                        :freq,:pulse_num_oneT, :rest_time, \
                        :T_num,:all_pulse_num, :all_time)");
    query.prepare(str);
    ...
    query.bindValue(":name",name);//病人名
    query.bindValue(":scheme", singledb.name);//方案名
    query.bindValue(":position",singledb.position);
    query.bindValue(":power",singledb.power);
    ...
    if(!query.exec())
        ...
        {   qDebug() << str;
            qDebug() << "History created!"<<singledb.name<<singledb.all_time;
            return true;
        }
}
